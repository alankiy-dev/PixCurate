        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                HelpSection(title: "表示モード", icon: "rectangle.grid.2x2") {
                    ...
                    HelpItem(label: "サムネイルが取得できない場合") {
                        "ファイルが読み込めずサムネイルを生成できない場合は、読み込み中のインジケーター（くるくる）ではなく写真アイコンのプレースホルダが表示されます。ディスク未接続やアクセス権がない場合に表示されます。"
                    }
                    HelpItem(label: "現像後JPEGをサムネイルに表示") {
                        "RAWファイルの隣（同じフォルダ）に同名のJPEG（例: DSCF5364.RAF に対する DSCF5364.jpg）があると、そのJPEGをサムネイル・拡大表示に使います。現像ソフトで書き出した現像後の絵をPixCurate上で確認できます。同名JPEGが無ければ、従来どおりRAWに埋め込まれた撮影時プレビューを表示します。★評価・タグなどのメタデータは従来どおりXMP優先です。"
                    }
                    HelpItem(label: "サムネイルの更新") {
                        "サムネイルはファイルの更新日時を見て管理され、現像ソフトでJPEGを書き出し直すと自動的に新しい絵に切り替わります。他アプリ（Bridge等）から戻ってPixCurateがアクティブになると、表示中のサムネイルを自動で読み直します。すぐ反映したいときは、ツールバーの「サムネイル更新」ボタン（写真に下矢印のアイコン。再スキャンとは別物）を押すと、変わったファイルだけ読み直します。"
                    }
                    HelpItem(label: "他アプリでの編集を自動反映") {
                        "Bridge や Lightroom 等で★評価やタグを付けて PixCurate に戻ると、アプリがアクティブになったタイミングで、変更されたXMPだけを自動で軽く読み直し、★やタグを反映します（全DB再構築より高速）。外付けドライブを走査しすぎないよう一定間隔で間引かれ、読み込み・コピー中は行いません。反映されない場合はツールバーの再スキャンで確実に更新できます。"
                    }
                    HelpItem(label: "サムネイルのクリック選択") {
                        "サムネイルはシングルクリックで即座に選択状態になります（ダブルクリックは拡大表示）。⌘クリックで追加選択、⇧クリックで範囲選択ができます。"
                    }
                ...
                HelpSection(title: "フィルター", icon: "line.3.horizontal.decrease.circle") {
                    ...
                    HelpItem(label: "評価フィルターのモード（以上／のみ）") {
                        "★の下に「以上」「のみ」のボタンがあります。「以上」はその評価以上の画像をすべて表示（例：★3以上 → ★3・★4・★5）。「のみ」はその評価の画像だけを表示（例：★3のみ → ★3だけ）。初期値は「以上」です。設定は次回起動時にも保持されます。"
                    }
                    HelpItem(label: "未評価（★なし）だけを表示") {
                        "★を0（すべて空）にして「のみ」を選ぶと、まだ評価していない未評価の画像だけを抽出できます。選別作業で「これから★を付ける写真」を探すのに便利です。★0＋「以上」は従来どおり全件表示（下限なし）です。"
                    }
                    ...
                    HelpItem(label: "季節フィルター（春夏秋冬）") {
                        "撮影日セクションの「季節」で、春・夏・秋・冬のチップをクリックすると、その季節に撮影した画像だけを表示します。複数選択すると OR 条件（例：春＋秋）。期間指定・例年フィルターとは独立した条件なので、単独でも他の条件と組み合わせても使えます。"
                    }
                    HelpItem(label: "季節の対象月を設定") {
                        "「季節」横の歯車アイコンから、各月（1〜12月）がどの季節に属するかを変更できます。初期値は春=3・4・5月／夏=6・7・8月／秋=9・10・11月／冬=12・1・2月。「初期値に戻す」ボタンでいつでも戻せます。設定は保存され、季節フィルターの判定に使われます。"
                    }
                    ...
                    HelpItem(label: "フィルターをすべて解除") {
                        "何か絞り込み中のとき、「フィルター」見出しの右端に「✕ すべて解除」ボタンが表示されます。クリックすると評価・フォーマット・カラーラベル・タグ・撮影地・季節・撮影日・更新日・プリセットをすべて解除し、全件表示に戻します（撮影日・更新日で入力した日付の値は保持されます）。"
                    }
                }
                ...
                HelpSection(title: "メタデータ編集", icon: "star.fill") {
                    ...
                    HelpItem(label: "現像アプリで開く（右クリック）") {
                        "右クリック→「開く」から、Photoshop・Bridge・Lightroom Classic などインストール済みの現像系アプリでファイルを開けます。複数選択中はまとめて開きます。Camera Rawは単体アプリではないため一覧には出ませんが、Adobe PhotoshopでRAWを開くとCamera Rawが起動します。「デフォルトアプリで開く」「ほかのアプリを選択…」も選べます。"
                    }
                    HelpItem(label: "拡大表示") {
                    ...
                    HelpItem(label: "拡大表示でのスクロール・ズーム") {
                        "ピクセル等倍などで画像が画面より大きい場合は中央に表示され、マウスのドラッグで上下左右にスクロール（パン）できます。トラックパッドのピンチで拡大、ダブルクリックで等倍に戻ります。"
                    }
                }
                ...
                HelpSection(title: "コピー機能", icon: "arrow.right.circle") {
                    ...
                    HelpItem(label: "同名ファイルもまとめてコピー") {
                        "コピー時は、RAW本体だけでなく同じフォルダにある同名ファイル（拡張子は問わず）をすべて一緒にコピーします。例えば DSCF5364.RAF をコピーすると、DSCF5364.xmp（現像情報）や DSCF5364.jpg（現像後JPEG）も同時に複製されます。ファイル名（拡張子を除く部分）が完全一致するものが対象で、接尾辞付き（例: DSCF5364_edit.jpg）は対象外です。プレビューには一緒にコピーされる関連ファイルも表示されます。"
                    }
                ...
                HelpSection(title: "コレクション", icon: "rectangle.stack") {
                    ...
                    HelpItem(label: "コレクションの階層管理（グループ）") {
                        "各コレクション行の「＋」ボタンを押すと、そのコレクションの中に子コレクションを作成できます。子を持つコレクションはフォルダアイコンの「グループ」として表示され、左のシェブロン（▸）で開閉できます。「コンテスト名 › 年度・号 › 部門」のように多段でまとめて整理できます。グループを削除すると、その中のすべての子コレクションも一緒に削除されます。"
                    }
                    HelpItem(label: "ファイルをコレクションに追加") {
                        "画像を右クリック→「コレクションに追加」→コレクション名を選択します。階層化している場合はグループがサブメニューとして表示され、目的のコレクションまでたどって追加できます。複数選択した状態で右クリックすると、選択したファイルをまとめて追加できます。「新規コレクションを作成して追加…」を選ぶと、コレクション作成と同時に追加できます。"
                    }
                    HelpItem(label: "コレクションを表示") {
                        "左パネルのコレクション名をクリックするとコレクション内のファイルが表示されます。グループ（子を持つ行）をクリックした場合は開閉のみで、画像を持つコレクションをクリックすると一覧が表示されます。表示中もフィルター機能は使用できます。"
                    }
                    HelpItem(label: "通常表示に戻る") {
                        "コレクション表示中は、写真一覧の上部に「✕ 〇〇を閉じる」ボタンが表示されます。クリックすると通常のフォルダ表示（コピー元フォルダの全画像）に戻ります。サイドバーで表示中のコレクションをもう一度クリックしても戻れます。"
                    }
                    HelpItem(label: "コレクションのファイルアクセス") {
                        "コレクションに画像を追加した時点で、そのファイルがあるフォルダへのアクセス権（security-scoped bookmark）が自動的に記録されます。これにより、後でコピー元フォルダの登録を変更・削除しても、コレクション内の画像のサムネイル表示・拡大表示が引き続き行えます。"
                    }
