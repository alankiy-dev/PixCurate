            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HelpSection(title: "フィルター", icon: "line.3.horizontal.decrease.circle") {
                    HelpItem(label: "フォーマット") {
                        "「RAW」「JPEG」「両方」の3つのアイコンボタンで表示対象のファイル種別を切り替えます。初期値は「RAW」です。"
                    }
                    HelpItem(label: "評価フィルター") {
                        "★をクリックするとその評価以上の画像を表示します。同じ★をもう一度クリックすると1段階下がります。★1をクリックした状態でもう一度クリックすると全件表示（評価なし含む）に戻ります。"
                    }
                ...
                HelpSection(title: "メタデータ編集", icon: "star.fill") {
                    ...
                    HelpItem(label: "撮影地の検索") {
                        "右パネル「撮影地」タブの検索フィールドに地名を入力すると、マスタ登録済みの撮影地を絞り込んで表示します。撮影地が増えてきた場合に素早く目的の地名を見つけられます。検索結果にはフルパス（例：大分県 › 由布市 › 金鱗湖）も表示されるため、同名の地名が別の場所にある場合でも区別できます。"
                    }
                    HelpItem(label: "タグの表示件数") {
                    ...
                    HelpItem(label: "拡大表示") {
                        "ダブルクリックまたは右クリック→「大きく表示」で画像を別ウィンドウに拡大表示します。縦位置画像も正しく縦向きで表示されます。"
                    }
                    HelpItem(label: "拡大表示での評価設定") {
                        "拡大表示ウィンドウでも 1〜5 キーで評価を設定、0 キーで解除できます。設定するとウィンドウ中央に★が大きく表示されてフェードアウトします。変更はメイン画面にも即座に反映されます。"
                    }
